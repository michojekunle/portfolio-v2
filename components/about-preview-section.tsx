  ...
  return (
    <section className="py-[120px] max-[720px]:py-[72px] max-w-[var(--maxw)] mx-auto px-[var(--gutter)]" aria-labelledby="about-preview-heading">
      ...
      >
        ...
        <div>
          <h2 id="about-preview-heading" className="m-0 font-display font-normal text-[clamp(44px,7vw,88px)] leading-[0.95] tracking-[-0.025em] text-[var(--ink)] text-balance fvs-display">
            ...
            <TextReveal delay={0.3} className="italic text-[var(--v3-accent)] fvs-soft">precision.</TextReveal>
          </h2>
          <motion.div 
            ...
            className="col-start-2 max-[720px]:col-start-1 max-w-[56ch] text-[17px] leading-[1.6] text-[var(--ink-2)] mt-[18px]"
          >
      ...
      <motion.div 
        ...
        transition={{ duration: 0.6 }}
        className="mt-[80px] pt-[40px] border-t border-[var(--rule)] flex justify-center"
      >
        <MagneticWrapper strength={20}>
          <Link href="/about" className="group inline-flex items-center justify-center px-[32px] h-[52px] rounded-full font-mono text-[11px] uppercase tracking-[0.15em] font-medium cursor-pointer border border-[var(--rule)] bg-transparent text-[var(--ink)] transition-all duration-300 no-underline hover:border-[var(--v3-accent)] hover:text-[var(--v3-accent)] hover:bg-[color-mix(in_oklab,var(--v3-accent)_5%,transparent)]">
            More about my journey <ArrowRight className="inline-block transition-transform duration-300 group-hover:translate-x-[4px] ml-[10px] w-3 h-3" aria-hidden="true" />
          </Link>
        </MagneticWrapper>
      </motion.div>
    </section>
