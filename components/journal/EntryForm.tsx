...
import { Plus, X, Save, Loader2, Zap, Sparkles } from "lucide-react";
...
export function EntryForm({ date, initialEntry, objectives, onSaved }: Props): React.ReactElement {
  ...
  return (
    <div className="space-y-[32px]">
      ...
      {/* ── Priorities ── */}
      <section>
        <label className="block font-mono text-[10px] tracking-[0.14em] uppercase mb-[12px] text-[var(--ink-3)]">
          Today's Priorities
        </label>
        <div className="space-y-[8px]">
          {priorities.map((p, i) => (
            <div
              key={i}
              className="flex items-center gap-[10px] group px-[14px] py-[10px] rounded-[8px]"
          ...
          <div className="flex gap-[8px]">
            <input
              ...
              className="flex-1 h-[40px] px-[14px] rounded-[8px] text-[14px] border border-[var(--rule)] bg-[var(--bg)] text-[var(--ink)] outline-none focus:border-[var(--ink-3)] transition-colors"
            />
            <button
      ...
      <section>
        ...
        <div className="space-y-[8px]">
          ...
          <div className="flex gap-[8px]">
            ...
            />
            <button
              onClick={() => addItem(accomplished, setAccomplished, newAccomplished, setNewAccomplished)}
      ...
      <section>
        <label
          htmlFor="blockers"
          className="block font-mono text-[10px] tracking-[0.14em] uppercase mb-[12px] text-[var(--ink-3)]"
        >
          Blockers & Friction
        </label>
        <textarea
          ...
          rows={3}
          className="w-full px-[14px] py-[12px] rounded-[8px] text-[14px] leading-[1.6] border border-[var(--rule)] bg-[var(--bg)] text-[var(--ink)] outline-none focus:border-[var(--ink-3)] transition-colors resize-none"
        />
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-[12px]">
          <label
            htmlFor="notes"
            className="block font-mono text-[10px] tracking-[0.14em] uppercase text-[var(--ink-3)] m-0"
          >
            Reflection & Notes
          </label>
          <button
            ...
            className="font-mono text-[8px] tracking-[0.08em] uppercase px-[8px] py-[4px] rounded-[6px] border border-[var(--rule)] bg-transparent cursor-pointer text-[var(--ink-3)] hover:text-[var(--ink)] hover:border-[var(--ink-2)] transition-all flex items-center gap-[4px]"
          >
        ...
        <div className="flex gap-[8px]">
          {[1, 2, 3, 4, 5].map((level) => (
            <button
              ...
              style={
                energy === level
                  ? { background: VELA_ACCENT, color: "#fff", borderColor: VELA_ACCENT }
                  : { background: "var(--bg-2)", color: "var(--ink-2)", borderColor: "var(--rule)" }
              }
              title={ENERGY_LABELS[level]}
            >
              <div className="text-[16px] mb-[2px]">{"⚡".repeat(level)}</div>
              <div className="font-mono text-[9px] tracking-[0.06em] hidden min-[480px]:block">
                {ENERGY_LABELS[level]}
              </div>
            </button>
          ))}
            ...
            {activeObjectives.map((obj) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-[7px] px-[12px] py-[7px] rounded-[8px] text-[13px] font-medium border transition-all cursor-pointer"
                  style={
                    active
                      ? { background: `${obj.color}18`, color: obj.color, borderColor: `${obj.color}40` }
                      : { background: "var(--bg-2)", color: "var(--ink-2)", borderColor: "var(--rule)" }
                  }
                >
                  <span>{obj.icon}</span>
      ...
      <button
        ...
        disabled={saving}
        className="flex items-center justify-center gap-[8px] w-full h-[48px] rounded-[10px] font-mono text-[11px] tracking-[0.12em] uppercase font-semibold text-white border-none cursor-pointer disabled:opacity-60 transition-all hover:opacity-90"
        style={{ background: saved ? "#16A34A" : VELA_ACCENT }}
      >
        {saving ? (
