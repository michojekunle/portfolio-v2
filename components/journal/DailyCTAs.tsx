...
type SlideId = "priorities" | "log";

const AUTO_ADVANCE_MS = 6000;

export function DailyCTAs({ date, todayEntry }: Props): React.ReactElement {
  // Default order is priorities-first — matches the server render and covers the
  // common morning/afternoon case. Flips to log-first once we know it's evening
  // (only knowable client-side), so evening visitors briefly see the reorder.
  const [order, setOrder] = useState<[SlideId, SlideId]>(["priorities", "log"]);
  const [active, setActive] = useState(0);

  useEffect(() => {
    const isEvening = new Date().getHours() >= 17;
    if (isEvening) setOrder(["log", "priorities"]);
  }, []);

  useEffect(() => {
    const id = setInterval(() => setActive((i) => (i + 1) % 2), AUTO_ADVANCE_MS);
