    ...
    >
      ...
      <div className="grid grid-cols-2 gap-10 items-start max-[920px]:hidden">
        ...
        <nav className="flex flex-col gap-3" aria-label="Featured projects">
          {FEATURED_PROJECTS.map((p, i) => (
            ...
            >
              <Link
                href={`/work/${p.slug}`}
                className={`group grid grid-cols-[24px_1fr_auto_auto_24px] gap-4 items-center p-[24px_20px] rounded-xl border border-transparent no-underline transition-all duration-300 ease-[cubic-bezier(0.2,0.8,0.2,1)] cursor-pointer bg-transparent text-(--ink) ${hoveredIdx === i ? "bg-(--bg-2) border-(--rule) opacity-100" : "opacity-75 text-secondary-foreground hover:opacity-100"}`}
                onMouseEnter={() => setHoveredIdx(i)}
                onFocus={() => setHoveredIdx(i)}
                aria-label={`View case study: ${p.name}`}
              >
                <span className="font-mono text-[12px] text-muted-foreground">{p.idx}</span>
                ...
                <span className="font-mono text-[12px] tracking-[0.08em] uppercase text-muted-foreground">{p.tag} · {p.year}</span>
                <span className="font-mono text-[12px] tracking-[0.08em] uppercase text-muted-foreground">{p.role.split(" · ")[0]}</span>
        ...
        <motion.div 
          ...
          className="sticky top-30 flex flex-col gap-3.5" 
          aria-label="Project visual preview"
        >
          <div className="aspect-[4/3] rounded-2xl overflow-hidden border border-(--rule) relative bg-(--bg-2)">
            {FEATURED_PROJECTS.map((p, i) => (
              <div key={p.slug} className={`absolute inset-0 transition-opacity duration-400 ease-in flex flex-col ${hoveredIdx === i ? "opacity-100 pointer-events-auto z-[2]" : "opacity-0 pointer-events-none"}`}>
                <div className="flex-1 relative overflow-hidden group">
                  {p.image ? (
                    ...
                    <div
                      className="v3-serif font-display"
                      style={{
                        width: "100%", height: "100%",
                        background: `linear-gradient(135deg, color-mix(in oklab, ${PROJECT_COLORS[p.slug] ?? "var(--v3-accent)"} 15%, #09090b) 0%, #09090b 100%)`,
                        display: "flex", alignItems: "center", justifyContent: "center",
                        fontSize: "36px", color: "#f5f5f4",
          ...
          <div className="flex justify-between items-center px-1">
            <span className="font-display text-[18px] text-(--ink)">{FEATURED_PROJECTS[hoveredIdx]?.name}</span>
            <span className="font-mono text-[12px] uppercase text-muted-foreground">{FEATURED_PROJECTS[hoveredIdx]?.tag} · {FEATURED_PROJECTS[hoveredIdx]?.year}</span>
          </div>
        </motion.div>
      </div>
          ...
          >
            ...
            >
              <div className="p-8 max-[480px]:p-6 flex flex-col gap-3 flex-1">
                <div className="font-mono text-[12px] uppercase text-muted-foreground tracking-widest">{p.idx} · {p.year} · {p.tag}</div>
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {p.stack.slice(0, 4).map((s) => (
                    <span key={s} className="font-mono text-[12px] text-muted-foreground uppercase">{s}</span>
                  ))}
                </div>
              </div>
            </Link>
      ...
      >
        <MagneticWrapper strength={20}>
          <Link href="/work" className="group inline-flex items-center justify-center px-8 py-3.5 rounded-full font-mono text-[12px] uppercase tracking-[0.15em] font-medium cursor-pointer border border-(--rule) bg-transparent text-(--ink) transition-all duration-300 no-underline hover:border-(--v3-accent) hover:text-(--v3-accent) hover:bg-[color-mix(in_oklab,var(--v3-accent)_5%,transparent)]">
