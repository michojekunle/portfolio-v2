...
export function UsesHeroWidget() {
  ...
  return (
    <div className="relative w-full max-w-[400px] max-[900px]:max-w-none rounded-[20px] border border-[var(--rule)] bg-[var(--paper)] p-[24px] overflow-hidden group shadow-[0_12px_40px_-12px_rgba(0,0,0,0.05)] backdrop-blur-md flex flex-col gap-4">
      ...
      <div className="absolute top-0 right-0 w-[140px] h-[140px] bg-gradient-to-br from-[var(--v3-accent-soft)] to-transparent rounded-full blur-[40px] opacity-40 pointer-events-none transition-all duration-500 group-hover:scale-125" />
      ...
      <div className="flex items-center justify-between pb-2 border-b border-[var(--rule)]">
        ...
        <div className="flex items-center gap-1 text-[var(--ink-3)] font-mono text-[9px] uppercase tracking-wider">
      ...
      {/* Terminal Code Area */}
      <div className="font-mono text-[11px] leading-[1.6] text-[var(--ink-2)] bg-[var(--bg-2)] p-4 rounded-[12px] border border-[var(--rule)] select-none">
        <div className="text-[var(--ink-3)]">{"{"}</div>
        {specLines.map((line, index) => (
          <div key={line.label} className="pl-4 flex">
            <span className="text-[var(--v3-accent)]">"{line.label}"</span>
            <span className="text-[var(--ink-3)] mx-1">:</span>
            <span className="text-emerald-600 dark:text-emerald-400 break-all">{line.value}</span>
            {index < specLines.length - 1 && <span className="text-[var(--ink-3)]">,</span>}
          </div>
        ))}
        <div className="text-[var(--ink-3)]">{"}"}</div>
      </div>
      ...
      {/* Prompt mimic */}
      <div className="flex items-center gap-1.5 font-mono text-[10px] text-[var(--ink-3)] pl-1">
        <span className="text-[var(--v3-accent)]">~</span>
        <span className="text-[var(--ink-2)]">$</span>
        <span className="text-[var(--ink)] animate-[cursor-blink_1.2s_step-start_infinite]">_</span>
      </div>
