...
function AccountCard({ account }: { account: FwAccount }): React.ReactElement {
  ...
  return (
    <div className="rounded-[14px] px-6 py-5" style={{ border: "1px solid var(--rule)", background: "var(--bg-2)" }}>
      ...
      <div className="text-[16px] font-medium text-(--ink) mb-0.5">{account.name}</div>
      {account.provider && <div className="font-mono text-[10px] text-muted-foreground mb-4">{account.provider}</div>}
      ...
      <div className="font-mono text-[10px] text-(--ink-4) mt-1">{account.currency} · current balance</div>
    </div>
  );
}
...
function AddAccountForm({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center px-0 sm:px-4">
      ...
      <div className="relative z-10 w-full max-w-[460px] rounded-t-[20px] sm:rounded-2xl overflow-hidden" style={{ background: "var(--bg)", border: "1px solid var(--rule)" }}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-(--rule)">
          <h2 className="font-display text-[20px] font-normal tracking-[-0.01em] fvs-text m-0 text-(--ink)">Add Account</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          {/* Account type */}
          <div>
            <label className="block font-mono text-[10px] tracking-[0.12em] uppercase text-muted-foreground mb-2">Account Type</label>
            <div className="grid grid-cols-5 gap-2">
              {(["bank","wallet","cash","investment","credit"] as FwAccount["type"][]).map((t) => (
                <button
                  ...
                  type="button"
                  onClick={() => setType(t)}
                  className="flex flex-col items-center gap-1 py-2.5 rounded-lg border-none cursor-pointer transition-all"
          ...
          {/* Name */}
          <div>
            <label className="block font-mono text-[10px] tracking-[0.12em] uppercase text-muted-foreground mb-2">Account Name</label>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. GTBank Main, OPay Wallet" required maxLength={60} className="w-full h-11 px-3.5 rounded-lg text-[14px] outline-none bg-(--bg-2) text-(--ink)" style={{ border: "1.5px solid var(--rule)" }} />
          </div>

          {/* Provider */}
          <div>
            <label className="block font-mono text-[10px] tracking-[0.12em] uppercase text-muted-foreground mb-2">Provider <span className="normal-case tracking-normal text-(--ink-4)">(optional)</span></label>
            <select value={provider} onChange={(e) => setProvider(e.target.value)} className="w-full h-11 px-3 rounded-lg text-[14px] outline-none bg-(--bg-2) text-(--ink) cursor-pointer" style={{ border: "1.5px solid var(--rule)" }}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block font-mono text-[10px] tracking-[0.12em] uppercase text-muted-foreground mb-2">Currency</label>
              <select value={currency} onChange={(e) => setCurrency(e.target.value as FwAccount["currency"])} className="w-full h-11 px-3 rounded-lg text-[14px] outline-none bg-(--bg-2) text-(--ink) cursor-pointer" style={{ border: "1.5px solid var(--rule)" }}>
                {["NGN","USD","GBP","EUR","GHS","KES"].map((c) => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label className="block font-mono text-[10px] tracking-[0.12em] uppercase text-muted-foreground mb-2">Starting Balance</label>
              <input type="number" value={balance} onChange={(e) => setBalance(e.target.value)} min="0" step="0.01" className="w-full h-11 px-3.5 rounded-lg text-[14px] outline-none bg-(--bg-2) text-(--ink)" style={{ border: "1.5px solid var(--rule)" }} />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {ACCOUNT_COLORS.map((c) => (
                <button key={c} type="button" onClick={() => setColor(c)} className="w-7 h-7 rounded-full border-none cursor-pointer transition-transform hover:scale-110" style={{ background: c, outline: color === c ? `3px solid ${c}` : undefined, outlineOffset: "2px" }} aria-label={c} />
              ))}
            </div>
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full h-12 rounded-[10px] font-mono text-[11px] tracking-[0.14em] uppercase font-semibold text-white border-none cursor-pointer disabled:opacity-60 flex items-center justify-center gap-2" style={{ background: ACCENT }}>
            {loading ? <span className="w-4 h-4 rounded-full border-0.5 border-white/30 border-t-white animate-spin" aria-hidden="true" /> : <><Check size={14} /> Add Account</>}
          </button>
        </form>
      </div>
    </div>
